...
$tcDiscPayloadSubdir = @()
foreach ($payload in $tcDiscPayload) {
    $tcDiscPayloadSubdir += @{ source = $payload.source; name = "tc\$($payload.name)" }
}
...
New-GameDisc 'tinycore11-desktop-6.18.33-game' 'Tiny Core 11 Desktop 6.18.33 Game Disc' 'artifacts\kernels\xbox-linux-6.18.33-fatx-tinycore-bzImage' 'akrn' 'artifacts\initramfs\xbox-tinycore-stage6-xfbdev-desktop.cpio' 'zinitrd' $tcAppend $tcDiscPayloadSubdir '6.18.33-fatx-tinycore' '' 268435456 '_pad'
...
$manifest = [ordered]@{
    ...
    notes = @(
        ...
        "Tiny Core 6.18 game ISO uses the small stage6 initramfs plus a tc subdirectory disc payload, and keeps the kernel/initrd root files named akrn/zinitrd so Xromwell still has root-level boot payloads while avoiding the large-initrd handoff failure and keeping default.xbe early.",
