...
build=/tmp/xbox-linux-5.8.1-noxpad-build
out=/mnt/c/Users/Paul/Desktop/xbox_linux/artifacts/kernels

case "$build" in
  /tmp/xbox-linux-5.8.1-noxpad-build) ;;
...
scripts/config --disable JOYSTICK_XPAD_FF
scripts/config --disable IP_NF_TARGET_ECN
scripts/config --disable NETFILTER_XT_TARGET_HL
make ARCH=x86 olddefconfig
grep -E 'CONFIG_USB_HID|CONFIG_HID_GENERIC|CONFIG_JOYSTICK_XPAD|CONFIG_INPUT_EVDEV|CONFIG_INPUT_JOYDEV' .config || true
make ARCH=x86 -j"$(nproc)" bzImage
cp arch/x86/boot/bzImage "$out/xbox-linux-5.8.1-noxpad-bzImage"
cp .config "$out/xbox-linux-5.8.1-noxpad.config"
ls -l "$out/xbox-linux-5.8.1-noxpad-bzImage"
